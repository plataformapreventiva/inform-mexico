...
* Checar ingesta de cngmd_transparencia y cngmd_participacion_ciudadana. "No existen"
* Revisar bien lo sel srt_pad
...
hospitales <- large_table(con,raw,recursos_hospitales) %>% 
  ...
  select(cve_muni,unidades,E13,E14,E15,E1819)
...
defunciones_grales <- large_table(con,raw,defunciones_generales) %>% 
  ...
  mutate(cve_muni_ocurr= as.integer(paste0(ent_ocurr,mun_ocurr))) %>% 
  select(cve_muni,edad,sexo,cve_muni_ocurr,causa_def)
...
cngmd_proteccion_civil <- large_table(con,raw,cngmd_proteccion_civil) %>%
  ...
  select(cve_muni, nd_nsnn1,nd_nsnn2,nd_spnn1,nd_nsnn4,nd_nsnn5,nd_nsnn6)

cngmd_drenaje <- large_table(con,raw,cngmd_drenaje) %>%
  mutate(cve_muni = as.integer(aps_folio)) %>% 
  select(cve_muni,sd_dren_cab,sd_dren_resto)
...
cngmd_recoleccion_basura <- large_table(con,raw,cngmd_recoleccion_basura) %>%
  ...
  select(cve_muni,basura_cant_ton,basura_porcent_cab)
...
# Join
capacidades <- left_join(cfe,internet,complejidad, by=c("cve_muni")) %>%
  left_join(indice_reglamentacion,indicadores_finanzas,by=c("cve_muni")) %>%
  left_join(hospitales,defunciones_grales, by=c("cve_muni")) %>%
  left_join(cngmd_proteccion_civil,cngmd_drenaje,by=c("cve_muni")) %>%
  left_join(cngmd_predial,cngmd_recoleccion_basura, by=c("cve_muni"))

# Construcción de Variables
# 1.Recoleccion de basura: %recolectado en cab vs resto
# 2. Densidad de medicos, hospitales y camas (falta población)
# 3. Separar mortalidad infantil y materna
