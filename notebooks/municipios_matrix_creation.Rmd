...
* Error que no supimos resolver en la ingesta de participacion_ciudadana (está pero no está).
* En proceso el índice del IMCO (sólo falta una de los 9 scripts de ingesta)
* Variables de Intercensal (Moniduarte las hizo) para join de amenazas. 
...
# cngmd_transparencia1 <- large_table(con,raw,cngmd_transparencia) %>% retrieve_result()
#   mutate(cve_muni = as.character(ubic_geo)) %>%
...
create_reports_2(capacidades %>% select(-totalpo1) %>% data.table(), 
               task_name = "capacidades", 
               output_dir = "../eda/reportes/capacidades")
```
